...
fn main() -> eframe::Result {
    env_logger::init();
    if let Some(path) = std::env::var_os("PEAKRUNNER_INTERIOR_SURVEY") {
        let which = std::env::var("PEAKRUNNER_SURVEY_MAP").unwrap_or_else(|_| "reference".into());
        if let Err(err) = peakrunner::interior_survey::run(std::path::Path::new(&path), &which) {
            eprintln!("{err}");
            std::process::exit(1);
        }
        std::process::exit(0);
    ...
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title(if std::env::var_os("PEAKRUNNER_MAP_PACK").is_some() {"PeakRunner — private map reference"} else {"PeakRunner"}),
