#!/usr/bin/env python3
"""Longfield: an original floodlit football stadium (the Football mode's first
map). Flat turf between two end zones 250 m apart, a skiable bowl, stands,
goal gates and floodlight masts. Original geometry, procedural terrain and
material kit only; the classic community stadiums were studied privately for
proportions (end-zone spacing, zone size, field width) and nothing else.
"""
...
import struct
import sys
from assets import longfield_stadium
from assets import longfield_materials
from assets import longfield_terrain
...
CENTRE = longfield_terrain.CENTRE
# Ball resets outside the bowl's rim.
BOUNDS_HALF = (longfield_terrain.HALF_W+longfield_terrain.BANK+3, longfield_terrain.HALF_L+longfield_terrain.BANK+3)


def build(output, bake=True):
    if output.exists(): raise ValueError('Refusing to overwrite existing pack')
    spec = json.loads((ROOT/'maps/longfield.json').read_text())
    ...
    for half in spec['halves']:
        ...
        anchors = longfield_stadium.build(mesh, team, centre_marks=team == 0)
        ...
        instances.append(dict(asset=longfield_stadium.ASSET_ID, team=team, yaw=half['yaw'],
            anchors={k: mesh.point(v) for k, v in anchors.items() if k != 'spawn_points'}))
    grid = longfield_terrain.heights(spec['seed'])
    heights = bytearray(struct.pack('<65536H', *[round(float(v)*32) for v in grid.ravel()]))
    weights = bytearray(longfield_terrain.weights(grid).tobytes())
    ...
    pack_writer.paint_materials(textures, count, ['concrete', 'panel', 'grate', 'trim', 'ember', 'glacier', 'light', 'bark'],
                                longfield_materials.texture, spec['seed'], kit)
    ...
    # No scattered props on a stadium; bake the terrain shade directly.
    ...
    manifest.update(version=1, id=spec['id'], name=spec['name'], flags=flags, spawns=spawns,
        ...
        football=dict(end_zones=zones, kickoff=kickoff, radius=longfield_stadium.ZONE_R,
                      bounds=[CENTRE-bx, CENTRE-bz, CENTRE+bx, CENTRE+bz]),
        terrain_shade=shade,
        asset_sha256=pack_writer.source_hash(longfield_stadium.__file__),
        terrain_source_sha256=pack_writer.source_hash(longfield_terrain.__file__),
        material_source_sha256=pack_writer.source_hash(longfield_materials.__file__),
        provenance='PeakRunner original stadium geometry, original procedural terrain and original material kit; no extracted assets or external height data',
        definition_sha256=pack_writer.source_hash(ROOT/'maps/longfield.json'))
...
if __name__ == '__main__':
    ...
    build(Path(args[0]) if args else ROOT/'assets/maps/longfield', bake='--no-bake' not in sys.argv)
