...
Repository: `~/workspace/peakrunner`. Three content directories: `src/`, `docs/`,
`research/`. **Run all Cargo, packaging and Docker commands below from `src/`:**
...
`src/` contains all build inputs, including required original map assets.
Private research is optional, ignored and not embedded in public binaries.
See [workspace layout](docs/workspace-layout.md). The old checkout remains
at `~/workspace/PeakRunner.bak` for recovery.
