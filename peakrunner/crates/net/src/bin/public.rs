...
async fn main() -> std::io::Result<()> {
    if std::env::args().any(|s| s == "--healthcheck") {
        peakrunner_net::public::http_get("http://127.0.0.1:8080/healthz", false)?;
        return Ok(());
    }
    env_logger::Builder::from_env(env_logger::Env::default().default_filter_or("info")).init();
    let directory = std::env::args().any(|s| s == "--directory");
    ...
    peakrunner_net::public::serve(&bind, directory, trusted,
