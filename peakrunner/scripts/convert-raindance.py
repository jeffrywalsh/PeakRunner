...
class Converter:
    ...
    def run(self, output):
        mission=self.resolve('Raindance_nef.mis')
        ...
        if len(names)!=4: raise ValueError('Expected four Raindance terrain layers')
        ...
        water=by_kind('WaterBlock')[0]['fields']
        water_layer=self.texture(water['surfaceTexture'])
        ...
        manifest=dict(version=1,name='Raindance — T2 Classic',flags=flags,spawns=spawns,holes=holes,
...
if __name__=='__main__':
    ...
    Converter(args.source,args.tools).run(args.output)
